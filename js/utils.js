...
NexT.utils = {
  ...
  registerCodeblock(element) {
    ...
    figure.forEach(element => {
      ...
      const languageName = [...element.classList].find(
        cls => cls !== 'highlight'
      );
      if (!inited) {
      ...
      const needFold =
        CONFIG.codeblock.fold.enable && height > CONFIG.codeblock.fold.height;
      if (
        !needFold &&
        !CONFIG.codeblock.copy_button.enable &&
        !CONFIG.codeblock.language
      )
        return;
      let target;
      if (CONFIG.hljswrap && CONFIG.codeblock.copy_button.style === 'mac') {
        ...
        if (!box) {
          ...
          const container =
            element.querySelector('.table-container') || element;
  ...
  setGutter(gap) {
    document.body.style.setProperty(
      '--dialog-scrollgutter',
      gap || `${window.innerWidth - document.body.clientWidth}px`
    );
  },
